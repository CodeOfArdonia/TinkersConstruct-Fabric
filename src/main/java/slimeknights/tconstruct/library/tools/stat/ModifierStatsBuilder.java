...
public class ModifierStatsBuilder {
  ...
  public <B> void updateStat(IToolStat<?> stat, Consumer<B> consumer) {
    consumer.accept((B) this.map.computeIfAbsent(stat, IToolStat::makeBuilder));
    this.dirty = true;
  }
  ...
  public void multiplier(INumericToolStat<?> stat, double value) {
    this.multipliers.put(stat, (float) (this.multipliers.getOrDefault(stat, 1f) * value));
  }
  ...
  public MultiplierNBT buildMultipliers(@Nullable Item filter) {
    ...
    for (Entry<INumericToolStat<?>, Float> entry : this.multipliers.entrySet()) {
