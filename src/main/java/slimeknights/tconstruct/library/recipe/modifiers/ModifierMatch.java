...
public abstract class ModifierMatch implements Predicate<List<ModifierEntry>> {
  ...
  protected static class TagMatch extends ModifierMatch {
    ...
    public boolean test(List<ModifierEntry> entries) {
      ...
      for (ModifierEntry entry : entries) {
        if (ModifierManager.isInTag(entry.getId(), this.tag) && entry.getLevel() >= this.level) {
          matches++;
        }
      }
    ...
    public JsonObject serialize() {
      ...
      if (this.required > 1) {
        json.addProperty("required", this.required);
      }
      return json;
    }
    ...
    public void write(FriendlyByteBuf buffer) {
      ...
      buffer.writeVarInt(this.required);
      if (this.required > 0) {
  ...
  protected static class ListMatch extends ModifierMatch {
    ...
    public JsonObject serialize() {
      ...
      json.addProperty("matches_needed", this.required);
    ...
    public void write(FriendlyByteBuf buffer) {
      if (this.options.size() == 1) {
        ...
        buffer.writeVarInt(this.options.size());
        buffer.writeVarInt(this.required);
        for (ModifierMatch match : this.options) {
          match.write(buffer);
        }
