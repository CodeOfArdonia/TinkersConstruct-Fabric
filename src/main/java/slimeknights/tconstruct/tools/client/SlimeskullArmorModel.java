...
public class SlimeskullArmorModel extends Model {
  ...
  public void renderToBuffer(PoseStack matrixStackIn, VertexConsumer vertexBuilder, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
    if (this.base != null) {
      ...
      matrixStackIn.translate(0.0D, this.base.young ? -0.015D : -0.02D, 0.0D);
      matrixStackIn.scale(1.01f, 1.1f, 1.01f);
      this.base.renderToBuffer(matrixStackIn, vertexBuilder, packedLightIn, packedOverlayIn, red, green, blue, alpha);
      matrixStackIn.popPose();

      if (this.headModel != null && this.headTexture != null && ArmorModelHelper.buffer != null) {
        VertexConsumer headBuilder = ItemRenderer.getArmorFoilBuffer(ArmorModelHelper.buffer, RenderType.entityCutoutNoCullZOffset(this.headTexture), false, this.hasGlint);
        matrixStackIn.pushPose();
        if (this.base.crouching) {
          matrixStackIn.translate(0, this.base.head.y / 16.0F, 0);
        }
        if (this.base.young) {
        ...
        this.headModel.setupAnim(0, this.base.head.yRot * 180f / (float) (Math.PI), this.base.head.xRot * 180f / (float) (Math.PI));
        this.headModel.renderToBuffer(matrixStackIn, headBuilder, packedLightIn, packedOverlayIn, red, green * 0.5f, blue, alpha * 0.8f);
        matrixStackIn.popPose();
      }
    }
  }
