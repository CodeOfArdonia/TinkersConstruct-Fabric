...
public class OverlordModifier extends Modifier {
  ...
  public void addVolatileData(ToolRebuildContext context, int level, ModDataNBT volatileData) {
    ...
    overslime.addCapacity(volatileData, this.getBoost(context.getBaseStats(), level, 0.10f * context.getDefinition().getData().getMultiplier(ToolStats.DURABILITY)));
  }

  @Override
  public void addToolStats(ToolRebuildContext context, int level, ModifierStatsBuilder builder) {
    // at most subtract 90% durability, note this runs before the tool durability modifier
    ToolStats.DURABILITY.add(builder, -this.getBoost(context.getBaseStats(), Math.min(level, 6), 0.15f));
  }
}
