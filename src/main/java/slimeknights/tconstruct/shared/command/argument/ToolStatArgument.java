...
public class ToolStatArgument<T extends IToolStat> implements ArgumentType<T> {
  ...
  public T parse(StringReader reader) throws CommandSyntaxException {
    ...
    if (!this.filter.isInstance(stat)) {
      throw WRONG_TYPE.createWithContext(reader, name, this.filter.getSimpleName());
    }
    return this.filter.cast(stat);
  }

  @Override
  public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
    return SharedSuggestionProvider.suggestResource(ToolStats.getAllStats().stream()
      .filter(this.filter::isInstance).<ResourceLocation>map(IToolStat::getName)::iterator, builder);
  }
