    ...
    if (!StringUtils.isEmpty(this.recipe) && ResourceLocation.isValidResourceLocation(this.recipe)) {
      ...
      if (recipe instanceof CraftingRecipe) {
        ...
        if (recipe instanceof ShapedRecipe shaped) {
          this.grid = new IngredientData[shaped.getHeight()][shaped.getWidth()];

          for (int y = 0; y < this.grid.length; y++) {
            for (int x = 0; x < this.grid[y].length; x++) {
              this.grid[y][x] = IngredientData.getItemStackData(NonNullList.of(ItemStack.EMPTY, ingredients.get(x + y * this.grid[y].length).getItems()));
            }
          }
        ...
        for (int i = 0; i < ingredients.size(); i++) {
          this.grid[i / h][i % w] = IngredientData.getItemStackData(NonNullList.of(ItemStack.EMPTY, ingredients.get(i).getItems()));
        }
      }
    }
  }
