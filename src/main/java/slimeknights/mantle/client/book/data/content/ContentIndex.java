...
public class ContentIndex extends ContentListing {
  ...
  public void build(BookData book, ArrayList<BookElement> list, boolean rightSide) {
    ...
    if (!this.loaded) {
      ...
      Set<String> hiddenSet = this.hidden == null
        ? ImmutableSet.of("hidden")
        : ImmutableSet.<String>builder().add("hidden").add(this.hidden).build();
      ...
      this.parent.parent.pages.forEach(page -> {
        ...
        if (page != this.parent && !IndexTransformer.isPageHidden(page) && !hiddenSet.contains(page.name)) {
          // perform extra action if anything happens before this page
          for (Operation operation : operations) {
            if (page.name.equals(operation.before)) {
              switch (operation.action) {
                case "add_group" -> this.addEntry(operation.data, null, true);
                case "column_break" -> this.addColumnBreak();
                case "line_break" -> this.addEntry("", null, false);
